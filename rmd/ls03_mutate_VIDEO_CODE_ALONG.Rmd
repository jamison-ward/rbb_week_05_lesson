...
```{r}
yao_height %>% 
  mutate(height_meters = height_cm / 100)  # Create a NEW column by dividing all entries in an existing column by 100
```
...
```{r}
yao_height %>% 
  mutate(height_cm = height_cm + 5)  # Overwrite an EXISTING column by adding 5 to all entries in the old column
```
...
Q_sarcopenia_resp_id <- 
  sarcopenia %>% 
  mutate(respondent_id = 1:n())  # Add respondent_id column to data frame

Q_sarcopenia_resp_id_arr <- Q_sarcopenia_resp_id %>%
  select(respondent_id, everything())  # Move respondent_id column to left of data frame
```
