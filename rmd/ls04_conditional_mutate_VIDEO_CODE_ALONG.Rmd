...
yaounde_educ %>%
  ...
  mutate(highest_educ_recode = case_when(highest_education %in% c("university", "Doctorate") ~ "Post-seconday"))
```
...
yaounde_educ %>%
  ...
  mutate(highest_educ_recode = case_when(highest_education %in% c("University", "Doctorate") ~ "Post-secondary",
...
yaounde_BMI <-
  ...
  mutate(BMI_classification = 
           case_when(BMI >= 18.5 & BMI < 25 ~ "Normal weight",
...
Q_adolescent_grouping <- 
  flu_linelist %>% 
  mutate(adolescent = case_when(age >= 10 & age <= 19 ~ "Yes",
                                TRUE ~ "No"))
```
...
yaounde_age_sex %>%
  mutate(recruit = case_when(
    age_years >= 20 & age_years <= 29 & sex == "Female" ~ "Recruit to female study",
    age_years >= 20 & age_years <= 29 & sex == "Male" ~ "Recruit to male study",
    TRUE ~ "Do not recruit"
  ))
```
...
Q_age_province_grouping <- 
  flu_linelist %>% 
  mutate(recruit = case_when(
    age >= 30 & age < 60 & province == "Jiangsu" ~ "Recruit to Jiangsu study",
    age >= 30 & age < 60 & province == "Zhejiang" ~ "Recruit to Zhejiang study",
    TRUE ~ "Do not recruit"
  ))
```
...
yaounde_symptoms_slice <- 
  yaounde %>% 
  select(symp_anosmia_or_ageusia, symp_cough) %>% 
